...
x = pKa()
#x.load_models("TrainDNN/models/uncleaned", "best.pt")
x.load_models("TrainDNN/models/uncleaned", "best_L1.pt")
print(x.Gmodels)
...
print(state, "DFT vs DNN")
print(result.loc["DFT"] - result.loc[model])
...
if os.path.exists(f"PoisonIvy_{state}.pkl"):
    print("Reloading:", f"PoisonIvy_{state}.pkl")
    data = pickle.load(open(f"PoisonIvy_{state}.pkl", 'rb'))
else:
    data = {}

if os.path.exists(f"Poison_{state}.xyz"):
    os.remove(f"Poison_{state}.xyz")
...
for i in tqdm.tqdm(h5):
    ...
    if i not in data:
        ...
        with open(f"PoisonIvy_{state}.pkl", 'wb') as f:
            pickle.dump(data, f)
    else:
        Range = data[i]["Range"]
        Mean = data[i]["Mean"]
    ...
    if ((Range*627.5) > cutoff).any() or (err > cutoff).any():
        ...
        for index in indices:
            mols[index].write(f"Poison_{state}.xyz", append=True)
            removed += 1
